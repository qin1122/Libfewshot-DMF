...
class DynamicWeights(nn.Module):
    def __init__(self, channels, dilation=1, kernel=3, groups=1, nFeat=640):
        ...
        self.unfold = nn.Unfold(kernel_size=(kernel, kernel),
                                padding=padding, dilation=1)  # 展平操作，将输入特征图中的局部区域展开为列
    ...
    def forward(self, t=None, x=None):
        ...
        xd_unfold = xd_unfold.permute(0, 1, 3, 2).contiguous().view(N, self.group, R, H * W, self.K).permute(0, 1, 3, 2,
                                                                                                             4).contiguous().view(
            N * self.group * H * W, R, self.K)  # 调整大小
        # 批量矩阵乘法，对一批矩阵中的每一对进行矩阵乘法。unsqueeze()函数为在指定维度上插入一维
        out1 = torch.bmm(xd_unfold, dynamic_filter.unsqueeze(2))
        out1 = out1.contiguous().view(N, self.group, H * W, R).permute(0, 1, 3, 2).contiguous().view(N, self.group * R, H * W).view(
            N, self.group * R, H, W)
...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        self.integration_time = self.integration_time.type_as(
            x[0])  # 将其数据类型转换为与输入x的类型相同
        out = odeint(self.odefunc, x, self.integration_time, rtol=1e-2, atol=1e-2,
                     method='rk4')  # 求解器，rtol和atol为相对和绝对误差限，rk4为四阶龙格库塔方法
...
class Model(nn.Module):
    def __init__(self, num_classes=64, nFeat=640, kernel=3, groups=1):
        ...
        self.nFeat = nFeat
        self.global_clasifier = nn.Conv2d(
            self.nFeat, num_classes, kernel_size=1)  # 图中的f_gc

        self.dw_gen = DynamicWeights_(self.nFeat, 1, kernel, groups)
        self.dw = self.dw = ODEBlock(DynamicWeights(
            self.nFeat, 1, kernel, groups, self.nFeat))
    ...
    def get_score(self, ftrain, ftest, num_train, num_test, batch_size):
        ...
        cls_scores = F.conv2d(cls_scores, conv_weight,
                              groups=b * n1 * n2, padding=1)  # 将计算出的得分与卷积权重卷积
    ...
    def forward(self, support_feat, query_feat, support_targets, query_targets, global_labels=None):
        ...
        prototypes = prototypes.contiguous().view(
            batch_size, -1, *original_feat_shape[2:])
        query_feat = query_feat.contiguous().view(
            batch_size, n_query, *original_feat_shape[2:])
        ...
        cls_scores = self.get_score(
            prototypes, query_feat, n_support, n_query, batch_size)

        if not self.training:
            return self.get_test_score(cls_scores)

        global_pred = self.get_global_pred(
            query_feat, query_targets, n_query, batch_size, K)
